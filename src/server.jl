...
const Cross_origin_JSON = Dict{Compat.UTF8String,Compat.UTF8String}("Content-Type" => "application/json; charset=utf-8", "Access-Control-Allow-Origin" => "http://localhost:8080") # No cross origin
const JSON_RESP_HDRS = Dict{String,String}("Content-Type" => "application/json; charset=utf-8") # With cross origin
...
# IMPORTANT: Register exposed functions here!
    # JuliaWebAPI.register(api, testfn1; resp_json=true, resp_headers=JSON_RESP_HDRS)
    # JuliaWebAPI.register(api, testfn2; resp_json=true, resp_headers=Cross_origin_JSON)
    # JuliaWebAPI.register(api, getNvList; resp_json=true, resp_headers=Cross_origin_JSON)
